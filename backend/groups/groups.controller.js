...
export const get_groups = async (req, res, next) => {
    try {
        ...
        const results = await groupsModel.find({ members: { $elemMatch: { 'user_id': tokenData._id, pending } } },
         { transactions: 0, members: 0 }).lean();
...
export const add_transaction = async (req, res, next) => {
    try {
        const { group_id } = req.params;
        let { title, description, category, amount, date, tokenData: { _id: user_id, fullname } } = req.body;
       ...
       return res.json({ success: true, data: results.modifiedCount ? true : false })
    } catch (error) {
        next(error)
    //    return res.json({ success: false, data: "Upload receipt" })
    }
}
